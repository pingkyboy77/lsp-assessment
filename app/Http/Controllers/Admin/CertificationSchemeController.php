...
class CertificationSchemeController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($query)
                ...
                ->addColumn('field_display', function ($row) {
                    if ($row->field) {
                        return '<div class="fw-semibold">' . $row->field->bidang . '</div>' . '<small class="text-muted">Kode: ' . $row->code_2 . '</small>';
                    }
                ...
                ->addColumn('status_badge', function ($row) {
                    ...
                    return '<span class="badge ' . $badgeClass . '">' . '<i class="bi ' . $icon . ' me-1"></i>' . $statusText . '</span>';
                })
                ->addColumn('action', function ($row) {
                    $statusAction = $row->is_active
                        ? '<a href="' .
                            route('admin.certification-schemes.toggle-status', $row->id) .
                            '"
                    ...
                    <div class="btn-group" role="group">
                        <!-- Requirements Button -->
                        <a href="' .
                        route('admin.certification-schemes.requirements', $row->id) .
                        '"
                        ...
                        <a href="' .
                        route('admin.schemes.unit-kompetensi.index', $row->id) .
                        '"
                        ...
                        <a href="' .
                        route('admin.schemes.kelompok-kerja.index', $row->id) .
                        '"
                        ...
                        <!-- View Button -->
                        <a href="' .
                        route('admin.certification-schemes.show', $row->id) .
                        '"
                        ...
                        <!-- Edit Button -->
                        <a href="' .
                        route('admin.certification-schemes.edit', $row->id) .
                        '"
                        ...
                        $row->id .
                        '" action="' .
                        route('admin.certification-schemes.destroy', $row->id) .
                        '"
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'code_2' => 'required|exists:fields,code_2',  // ✅ BETUL - cek di kolom 'code_2'
        ...
        try {
            ...
            return redirect()->route('admin.certification-schemes.index')->with('success', 'Skema sertifikasi berhasil ditambahkan.');
        } catch (\Exception $e) {
    ...
    {
        $templates = RequirementTemplate::with([
    ...
    public function update(Request $request, CertificationScheme $certificationScheme)
    {
        ...
        $request->validate([
            ...
            'nama' => 'required|string|max:255',
            'skema_ing' => 'nullable|string|max:255',
            'code_2' => 'required|exists:fields,code_2',  // ✅ BETUL 
            'jenjang' => 'required|in:Utama,Madya,Menengah',
            'is_active' => 'boolean',
        ]);
        ...
        try {
            ...
            return redirect()->route('admin.certification-schemes.index')->with('success', 'Skema sertifikasi berhasil diperbarui.');
        } catch (\Exception $e) {
    ...
    public function destroy(CertificationScheme $certificationScheme)
    {
        try {
            ...
            $hasRelatedData = $certificationScheme->unitKompetensis()->exists() || $certificationScheme->kelompokKerjas()->exists() || $certificationScheme->requirementTemplates()->exists();
            ...
            // ]);
            return redirect()->route('admin.certification-schemes.index')->with('success', 'Skema sertifikasi berhasil diha[us.');
        } catch (\Exception $e) {
    ...
    public function updateRequirements(Request $request, CertificationScheme $certificationScheme)
    {
        ...
        try {
            ...
            // Handle multiple templates dengan pivot table
            if ($request->has('requirement_templates') && !empty($request->requirement_templates)) {
                $syncData = [];
                ...
                // Gunakan sync untuk mengganti semua relasi dengan data baru
                $certificationScheme->requirementTemplates()->sync($syncData);
            } else {
                // Jika tidak ada templates yang dipilih, hapus semua relasi
                $certificationScheme->requirementTemplates()->detach();
            }

            DB::commit();

            return redirect()->route('admin.certification-schemes.requirements', $certificationScheme)->with('success', 'Template persyaratan berhasil diperbarui.');
        } catch (\Exception $e) {
            DB::rollBack();
            ...
            return redirect()
                ->back()
                ->with('error', 'Terjadi kesalahan saat memperbarui template persyaratan: ' . $e->getMessage());
        }
    }
    ...
    public function resetRequirements(CertificationScheme $certificationScheme)
    {
        try {
            ...
            return redirect()->route('admin.certification-schemes.requirements', $certificationScheme)->with('success', 'Semua template persyaratan berhasil direset.');
        } catch (\Exception $e) {
            DB::rollBack();
            ...
            return redirect()
                ->back()
                ->with('error', 'Terjadi kesalahan saat mereset template persyaratan: ' . $e->getMessage());
        }
    }

    // Method alternative menggunakan method dari model
    public function updateRequirementsUsingModel(Request $request, CertificationScheme $certificationScheme)
    {
        $request->validate([
            'requirement_templates' => 'nullable|array',
            'requirement_templates.*' => 'exists:requirement_templates,id',
        ...
        try {
            ...
            // Handle multiple templates menggunakan method dari model
            if ($request->has('requirement_templates') && !empty($request->requirement_templates)) {
                // Gunakan method syncRequirementTemplates dari model
                $certificationScheme->syncRequirementTemplates($request->requirement_templates);
            } else {
    ...
    public function removeRequirementTemplate(Request $request, CertificationScheme $certificationScheme)
    {
        $request->validate([
        ...
        try {
            $templateId = $request->requirement_template_id;

            $result = $certificationScheme->removeRequirementTemplate($templateId);

            if ($result) {
                return redirect()->route('admin.certification-schemes.requirements', $certificationScheme)->with('success', 'Template persyaratan berhasil dihapus.');
            } else {
                return redirect()->back()->with('error', 'Template persyaratan tidak ditemukan.');
            }
        } catch (\Exception $e) {
            Log::error('Error removing requirement template: ' . $e->getMessage());

            return redirect()->back()->with('error', 'Terjadi kesalahan saat menghapus template persyaratan.');
        }
    }

    // Method untuk reorder templates
    public function reorderRequirementTemplates(Request $request, CertificationScheme $certificationScheme)
    {
        $request->validate([
            'template_ids' => 'required|array',
            'template_ids.*' => 'exists:requirement_templates,id',
        ]);

        try {
            DB::beginTransaction();

            $result = $certificationScheme->reorderRequirementTemplates($request->template_ids);

            if ($result) {
                DB::commit();
                return response()->json([
                    'success' => true,
                    'message' => 'Urutan template berhasil diperbarui.',
                ]);
            }
            ...
            return response()->json(
                [
                    'success' => false,
                    'message' => 'Gagal memperbarui urutan template.',
                ],
                400,
            );
        } catch (\Exception $e) {
            DB::rollBack();
            Log::error('Error reordering requirement templates: ' . $e->getMessage());

            return response()->json(
                [
                    'success' => false,
                    'message' => 'Terjadi kesalahan saat memperbarui urutan template.',
                ],
                500,
            );
        }
    }
    // Method untuk menambah single template
    public function addRequirementTemplate(Request $request, CertificationScheme $certificationScheme)
    {
        $request->validate([
            'requirement_template_id' => 'required|exists:requirement_templates,id',
        ]);
        ...
        try {
            ...
            if ($certificationScheme->requirementTemplates()->where('requirement_template_id', $templateId)->exists()) {
                return redirect()->back()->with('warning', 'Template persyaratan sudah ditambahkan sebelumnya.');
            }

            // Tambah template baru
            $result = $certificationScheme->addRequirementTemplate($templateId);

            if ($result) {
                return redirect()->route('admin.certification-schemes.requirements', $certificationScheme)->with('success', 'Template persyaratan berhasil ditambahkan.');
            } else {
                return redirect()->back()->with('error', 'Gagal menambahkan template persyaratan.');
            }
        } catch (\Exception $e) {
            Log::error('Error adding requirement template: ' . $e->getMessage());

            return redirect()->back()->with('error', 'Terjadi kesalahan saat menambahkan template persyaratan.');
        }
    }
