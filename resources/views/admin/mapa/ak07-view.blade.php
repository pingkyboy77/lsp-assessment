    ...
    <div class="container-fluid p-4">
        <div class="page-header">
            <div class="d-flex justify-content-between align-items-center">
                <div>
                    <h4><i class="bi bi-eye me-2"></i>Detail FR.AK.07 - Admin View</h4>
                    <p class="mb-0 opacity-90">Ceklis Penyesuaian Yang Wajar dan Beralasan</p>
                </div>
                <a href="{{ route('admin.mapa.index', $mapa->id) }}" class="btn btn-light">
                    <i class="bi bi-arrow-left me-1"></i>Kembali ke Review MAPA
                </a>
            </div>
        </div>
        ...
        <div class="main-card">
            ...
            <div class="detail-section">
                ...
                <div class="info-row">
                    <div class="row g-3">
                        <div class="col-md-6">
                            <div class="data-row">
                                <label class="small text-muted mb-1">Nomor MAPA</label>
                                <div class="fw-semibold">{{ $mapa->nomor_mapa }}</div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="data-row">
                                <label class="small text-muted mb-1">Nama Asesi</label>
                                <div class="fw-semibold">{{ $mapa->delegasi->asesi->name }}</div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="data-row">
                                <label class="small text-muted mb-1">Skema Sertifikasi</label>
                                <div class="fw-semibold">{{ $mapa->certificationScheme->nama }}</div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="data-row">
                        ...
                        <div class="col-md-6">
                            <div class="data-row">
                                <label class="small text-muted mb-1">Status AK07</label>
                                <div>
                                    @if ($ak07->status === 'completed')
                                        <span class="badge bg-success">Completed</span>
                                    @elseif($ak07->status === 'waiting_asesi')
                                        <span class="badge bg-warning">Menunggu Asesi</span>
                                    @elseif($ak07->status === 'draft')
                                        <span class="badge bg-secondary">Draft</span>
                                    @else
                                        <span class="badge bg-info">{{ ucfirst($ak07->status) }}</span>
                                    @endif
                                </div>
                            </div>
                        </div>
            ...
            @elseif($ak07->status === 'draft')
                <div class="alert alert-secondary mb-4 m-3">
                    <i class="bi bi-pencil-square me-2"></i>
                    <strong>Status:</strong> Draft - Masih dalam pengerjaan
                </div>
            @endif
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 mb-3">
                        <div class="card border">
                            ...
                            <div class="card-body text-center">
                                <div class="mb-2">
                                    <strong>{{ $mapa->delegasi->asesi->name }}</strong>
                                </div>
            ...
            <div class="action-buttons">
                <a href="{{ route('admin.mapa.show', $mapa->id) }}" class="btn btn-outline-secondary">
                    <i class="bi bi-arrow-left me-1"></i>Kembali ke Review MAPA
                </a>
                <button type="button" class="btn btn-primary" onclick="window.print()">
                    <i class="bi bi-printer me-1"></i>Cetak AK07
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    window.addEventListener('beforeprint', function() {
        ...
        document.querySelectorAll('.action-buttons')?.forEach(el => el.classList.add('d-print-none'));
    });
</script>
@endpush